...
static int __init char_null_udev_init(void)
{
        ...
        if(major > 0){          //静态设备号
                devno = MKDEV(major,minor);     //将主次设备号转换成设备编号
        ...
        char_null_udev = cdev_alloc();                  //分配char_null_udev结构
        if(char_null_udev != NULL){
                cdev_init(char_null_udev,&major);       //初始化char_null_udev结构
                char_null_udev->owner = THIS_MODULE;
                if(cdev_add(char_null_udev,devno,1) != 0){//增加char_null_udev到系统中
